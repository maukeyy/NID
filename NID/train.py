...
def test_one_epoch(test_model):
  ...
  print("\r                                                                    "
        "                                                                      \r", end="")
  t2 = time.time()
  return TestOutputs(csig=pm[0], cbak=pm[1], covl=pm[2], pesq=pm[3], ssnr=pm[4], stoi=pm[5],
                     cost_time=t2-t1)
...
def main():
  ...
  for epoch in range(s_epoch, max_epoch+1):
    ...
    misc_utils.print_log("  Test      > Csig: %.3f, Cbak: %.3f, Covl: %.3f, pesq: %.3f,"
                         " ssnr: %.4f, stoi: %.4f, Time:%ds.           \n" % (
                             testOutputs.csig, testOutputs.cbak, testOutputs.covl, testOutputs.pesq,
                             testOutputs.ssnr, testOutputs.stoi, testOutputs.cost_time),
                         train_log_file)
...
if __name__ == "__main__":
  ...
  """
  run cmd:
  `CUDA_VISIBLE_DEVICES=1 OMP_NUM_THREADS=1 python -m xx._2_train`
  `CUDA_VISIBLE_DEVICES=0,1 python -m PHASEN_009._2_train`
  """
